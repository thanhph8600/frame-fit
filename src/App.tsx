  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100">
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        {status === 'done' && analysis && recommendation && (
          <div className="mt-8">
            <div className="text-center mb-6">
              ...
              <p className="text-neutral-600 dark:text-neutral-300 mt-2 max-w-lg mx-auto">
                {FACE_SHAPE_INFO[analysis.faceShape].description}
              </p>
              {analysis.alternateFaceShape && (
                <p className="text-xs text-amber-600 dark:text-amber-400 mt-3 max-w-lg mx-auto">
                  Khuôn mặt bạn nằm gần ranh giới giữa dáng{' '}
                  {FACE_SHAPE_INFO[analysis.faceShape].label.toLowerCase()} và dáng{' '}
                  {FACE_SHAPE_INFO[analysis.alternateFaceShape].label.toLowerCase()} — ảnh khác góc
                  chụp có thể cho kết quả nghiêng về dáng còn lại. Gợi ý bên dưới đã cân nhắc cả
                  hai.
                </p>
              )}
            </div>

            <div className="mb-10 max-w-lg mx-auto rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
              <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-3 text-center">
                Số đo chi tiết
              </p>
              <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
                <span className="text-neutral-500 dark:text-neutral-400">Khoảng cách hai mắt</span>
                ...
                {m && (
                  <>
                    <span className="text-neutral-500 dark:text-neutral-400">Chiều rộng khuôn mặt</span>
                    <span className="text-right font-medium">{formatMm(m.faceWidth)}</span>
                    <span className="text-neutral-500 dark:text-neutral-400">Chiều dài khuôn mặt</span>
                    <span className="text-right font-medium">{formatMm(m.faceLength)}</span>
                    <span className="text-neutral-500 dark:text-neutral-400">Chiều rộng hàm</span>
                    <span className="text-right font-medium">{formatMm(m.jawWidth)}</span>
                    <span className="text-neutral-500 dark:text-neutral-400">Chiều rộng miệng</span>
                    <span className="text-right font-medium">{formatMm(m.mouthWidth)}</span>
                  </>
                )}
                {analysis.recommendedFrameWidthMm && (
                  <>
                    <span className="text-neutral-500 dark:text-neutral-400">Cỡ gọng nên chọn</span>
                    <span className="text-right font-medium">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-10">
              {recommendation.recommended.map((item) => (
                <GlassesCard key={item.id} item={item} highlighted />
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {recommendation.others.map((item) => (
                <GlassesCard key={item.id} item={item} />
              ))}
            </div>
          </div>
        )}
